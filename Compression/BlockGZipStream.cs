...
namespace Compression
{
    ...
    public sealed class BlockGZipStream : Stream
    {
        ...
        public BlockGZipStream(Stream stream, CompressionMode compressionMode, bool leaveStreamOpen = false, int compressionLevel = 1)
        {
            ...
            if (_isCompressor  && !_stream.CanWrite) throw new InvalidOperationException("A stream lacking write capability was provided to the block GZip compressor.");
            if (!_isCompressor && !_stream.CanRead)  throw new InvalidOperationException("A stream lacking read capability was provided to the block GZip decompressor.");
        ...
        private void ReadBlock()
        {
            ...
            {
                throw new InvalidDataException($"Found an invalid header when reading the GZip block ({_filePath})");
            }
            ...
            if (count != remaining)
            {
                throw new InvalidDataException($"Found unexpected truncation when reading the GZip block ({_filePath})");
            }
            ...
            if (count < 0)
            {
                throw new InvalidDataException($"Encountered an error when uncompressing the GZip block ({_filePath})");
            }
        ...
        {
            if (_isCompressor) throw new InvalidOperationException("Tried to read data from a compression BlockGZipStream.");
        ...
        {
            if (!_isCompressor) throw new InvalidOperationException("Tried to write data to a decompression BlockGZipStream.");
